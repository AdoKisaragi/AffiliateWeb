(function () {
  ...
  if (categoryPage) {
    ...
    const categoryUrl = `https://adokisaragi.github.io/AffiliateWeb/category.html?category=${encodeURIComponent(c.id)}`;
    const canonical = document.querySelector('link[rel="canonical"]');
    if (canonical) canonical.href = categoryUrl;
    const updateMeta = (selector, value) => {
      const meta = document.querySelector(selector);
      if (meta) meta.content = value;
    };
    updateMeta('meta[property="og:title"]', `${c.name}｜CHOICE LAB`);
    updateMeta('meta[property="og:description"]', c.description);
    updateMeta('meta[property="og:url"]', categoryUrl);
    updateMeta('meta[name="twitter:title"]', `${c.name}｜CHOICE LAB`);
    updateMeta('meta[name="twitter:description"]', c.description);
    const breadcrumbs = document.createElement("script");
    breadcrumbs.type = "application/ld+json";
    breadcrumbs.textContent = JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"ホーム","item":"https://adokisaragi.github.io/AffiliateWeb/"},{"@type":"ListItem","position":2,"name":c.name,"item":categoryUrl}]});
    document.head.append(breadcrumbs);
    ...
    const empty = (title) => `<div class="visual-empty">${image("assets/images/common/no-results.svg", "該当する掲載情報がないことを表すイラスト", "empty-image")}<p>${title}</p></div>`;
    categoryPage.innerHTML = `<header class="page-header category-header"><div class="container category-header-grid"><div><nav class="breadcrumb"><a href="index.html">ホーム</a> / カテゴリー / ${c.name}</nav><div class="eyebrow">CATEGORY</div><h1>${c.name}</h1><p class="lead">${c.description}</p><p><strong>選ぶときの確認ポイント：</strong>${c.filters.join("・")}</p></div>${image(c.image, c.imageAlt, "category-hero-image", false)}</div></header>${subcategorySection}<section class="section"><div class="container"><div class="affiliate-box"><strong>広告について</strong><p>当サイトはA8.netのアフィリエイト広告を利用しています。最新の販売情報は販売ページでご確認ください。</p></div><div class="section-head"><div><h2>選び方・比較記事</h2></div></div><div class="articles-grid">${relatedArticles.length ? relatedArticles.map(a => `<article class="article-card"><div class="article-body"><div class="eyebrow">${a.type}</div><h3>${a.title}</h3><p>${a.description}</p><a href="${a.url}">記事を読む →</a></div></article>`).join("") : empty("このカテゴリーの記事は現在掲載していません。")}</div></div></section><section class="section section-soft"><div class="container"><div class="section-head"><div><h2>関連商品</h2></div></div><div class="product-list">${relatedProducts.map(p => `<article class="simple-product">${image(p.image.src, p.image.alt, "")}<div><span>${p.subCategory}</span><h3>${p.name}</h3><p>${p.shortDescription}</p><a href="product-detail.html?id=${p.id}">商品情報を確認する →</a></div></article>`).join("") || empty("このカテゴリーの商品は現在掲載していません。")}</div><h2>関連カテゴリー</h2><div class="need-grid">${categories.filter(x => x.id !== c.id).slice(0, 4).map(x => `<a class="need-card" href="category.html?category=${x.id}">${x.name}</a>`).join("")}</div></div></section>`;
  }
})();
